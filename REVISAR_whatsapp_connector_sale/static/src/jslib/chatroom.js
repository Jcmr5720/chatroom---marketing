...
this.saleAllowed=false},getSubEnv(){let localObj={}
const out=this._super()
out.saleAllowed=()=>this.saleAllowed
out.saleAddToOrder=product=>{localObj.product=product
this.env.chatBus.trigger('updateTab','tab_order')}
out.saleGetProductoToOrder=()=>{const{product}=localObj
localObj.product=null
return product}
return out},async willStart(){return Promise.all([this._super(),this.env.services.user.hasGroup('sales_team.group_sale_salesman').then(res=>{this.saleAllowed=res}),this.getSaleFormView().then(res=>{this.saleFormView=res})])},async getSaleFormView(){const{orm}=this.env.services
...
return __exports;});;
odoo.define('@976a29642646c1c360d2f61e3b6808b047f1b05ced535217070aa6a394ac4d05',async function(require){'use strict';let __exports={};const{patch}=require('@web/core/utils/patch')
const{ProductContainer}=require('@aedb85b64f8970ed4ccdcfb5fad7484eb5f9502792073b672b574c2d95ef5fe2')
const saleProductOptions={async doProductOption({product,event}){let out=Promise.resolve()
if(event.target.classList.contains('acrux_add_to_order')){this.env.saleAddToOrder(product)}else{out=this._super({product,event})}
return out}}
patch(ProductContainer.prototype,'saleProductOptions',saleProductOptions)
return __exports;});;
...
this.env.chatBus.on('chatroomAddToOrder',this,this.acruxAddProductToLine)
onWillStart(async()=>{const product=this.env.saleGetProductoToOrder()
if(product){await this.acruxAddProductToLine(product)
await this.props.record.save()
await this.props.saveButtonClicked()}})
onWillDestroy(()=>this.env.chatBus.off('chatroomAddToOrder',this))}
async acruxAddProductToLine(product){if(this.props.record.data.state==='draft'){const context={...this.env.services.user.context,default_product_id:parseInt(product.id)}
